...
import css from './Searchpagelink.css';
import { createSearchResultSchema } from '../../containers/SearchPage/SearchPage.helpers';

const MIN_LENGTH_FOR_LONG_WORDS = 16;

const SearchpageLink = props => {
  const { className, listing, children } = props;
  const listingLoaded = listing && listing.id;
  if (!listingLoaded) {
    return null;
  }
  const id = createSearchResultSchema.id.uuid;
